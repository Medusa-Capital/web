...
- **Categories:** `DEX`, `AI_DEPIN`, `L1`, `L2`, `RESTAKING`, `RWA`, `PERPS`, `LENDING`, `INFRASTRUCTURE`. URL-safe / uppercase. Spanish labels in `enums/category.ts` (`Lending`, `Infraestructura`, etc.). Adding a new category requires: (1) push to `CATEGORY_VALUES`, (2) add label/tone/dot entries, (3) `ALTER TYPE category ADD VALUE` migration.
- **Chains:** single primary chain per analysis: `Ethereum`, `Base`, `Solana`, `Arbitrum`, `Optimism`, `Bitcoin`, `Hyperliquid_L1`, `Other`. **Multichain projects** record their primary chain (deepest deployment / canonical home) in the `chain` field and surface secondary chains as `multichain:<Chain>` entries in `tags` (e.g. Morpho on Ethereum + Base → `chain: "Ethereum"`, `tags: ["multichain:Base", ...]`). The `chain` array refactor is tracked as a follow-up — see `docs/sistema-medusa/multichain-followup.md`.
- **Token branding:** logos live at `public/img/tokens/<ticker>.png` (256×256, transparent-friendly). Brand colors registered per ticker in `lib/sistema-medusa/token-registry.ts` — used for ambient glows, avatar tint, accent strips. Verdict colors are independent of brand color.
- **Source URL policy:** every entry in `sources.primary` and `sources.secondary` must have an `https://` URL. Internal Medusa research with no public URL is **dropped** at ingest time (do not stub with placeholder URLs). When an internal artifact gets published (blog, lead magnet, public research), add it back as a real source.
